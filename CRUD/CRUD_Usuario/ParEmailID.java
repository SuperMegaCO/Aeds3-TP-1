...
public class ParEmailID implements aed3.RegistroHashExtensivel<ParEmailID> {
    ...
    public byte[] toByteArray() throws IOException {
        ...
        for (int i = 0; i < Math.min(emailBytes.length, 40); i++) {
        buffer[i] = emailBytes[i];
        }      
        dos.write(buffer);
    ...
    public static int hash(String Email) throws IllegalArgumentException {
        // Certifique-se de que o Email contém exatamente 40 dígitos
        long EmailLong = UUID.nameUUIDFromBytes(Email.getBytes()).getMostSignificantBits();
